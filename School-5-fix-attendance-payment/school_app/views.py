...
def student_detail(request, student_id):
    ...
    for group in enrolled_groups_qs:
        ...
        if price_per_session > Decimal('0.00'):
            if student_group and student_group.balance > Decimal('0.00'):
        ...
        enrolled_groups_with_payment_stats.append({
            ...
            'balance': student_group.balance if student_group else 0,
            ...
            if was_paid:
                ...
                if group.price_per_4_sessions and group.price_per_4_sessions > Decimal('0'):
                    ...
                    try:
                        ...
                        messages.error(request, "لم يتم العثور على تسجيل الطالب في الفوج لإعادة المبلغ.")
...
def student_monthly_payment_view(request, student_id):
    ...
    student_group_balance = Decimal('0.00')
    ...
    if selected_group_id:
        try:
            ...
            student_group_balance = student_group.balance
            ...
            if student_group.is_free:
                gross_amount_due = Decimal('0.00')
                attended_but_not_paid_sessions_count = 0 # Reset this as well
            else:
            ...
            net_amount_due = gross_amount_due - student_group_balance
            if net_amount_due < Decimal('0.00'):
                net_amount_due = Decimal('0.00')

            # Calculate prepaid_sessions_count
            if price_per_session > Decimal('0.00') and student_group_balance > Decimal('0.00'):
                import math # Ensure math is imported
                prepaid_sessions_count = math.floor(student_group_balance / price_per_session)
            else:
                prepaid_sessions_count = 0
    ...
    if request.method == 'POST':
        ...
        if action == 'mark_excused':
                        ...
                        with transaction.atomic():
                            ...
                            if was_paid:
                                if current_price_per_session_post > Decimal('0'):
                                    try:
                                        student_group_to_refund = StudentGroup.objects.get(student=student, group=attendance_to_excuse.session.group)
                                        student_group_to_refund.balance += current_price_per_session_post
                                        student_group_to_refund.save(update_fields=['balance'])
                                        ...
                                        messages.error(request, "خطأ: لم يتم العثور على تسجيل الطالب في الفوج لإعادة المبلغ.")
                                else:
            ...
            try:
                ...
                if student_group_for_payment.is_free:
                    if amount_paid_from_form > Decimal('0.00'):
                        student_group_for_payment.balance += amount_paid_from_form
                ...
                if student_group_for_payment.balance > Decimal('0.00'):
                    messages.info(request, f"تم استخدام رصيد مدفوع مقدماً بقيمة: {student_group_for_payment.balance} دج.")
                    amount_paid += student_group_for_payment.balance
                    prepaid_used_this_transaction = student_group_for_payment.balance
                    student_group_for_payment.balance = Decimal('0.00')
                ...
                if amount_paid <= 0: # Check effective amount_paid
                    ...
                    if current_balance > Decimal('0.00') and current_price_per_session_post > Decimal('0.00'):
                        student_group_for_payment.balance += current_balance
                        ...
                        if not student_group_for_payment.balance == (amount_paid - prepaid_used_this_transaction + current_balance): # Avoid double message if already handled by above
                             student_group_for_payment.balance += current_balance # This is the original amount_paid if nothing was spent
                             messages.info(request, f"لا توجد حصص لدفعها أو المبلغ غير كافٍ. تم إضافة {current_balance.quantize(Decimal('0.01'))} دج بالكامل إلى رصيد الطالب في الفوج.")
            ...
            try:
                ...
                if student_group_for_reduction.balance >= amount_to_deduct:
                    ...
                    log_action(
                        ...
                        f"Reduced balance for group {group_for_price_info.name} by {amount_to_deduct} DZD. Old balance: {old_balance}, New balance: {student_group_for_reduction.balance}"
                    )
                    messages.success(request, f"تم تخفيض رصيد الفوج بنجاح بمقدار {amount_to_deduct} دج.")
                else:
                    messages.error(request, f"لا يمكن تخفيض الرصيد. الرصيد الحالي للفوج ({student_group_for_reduction.balance} دج) أقل من المبلغ المراد خصمه ({amount_to_deduct} دج).")
    ...
    context = {
        ...
        'student_group_balance': student_group_balance,
                    ...
                    with transaction.atomic():
                        ...
                        if was_paid:
                            ...
                            if group_price and group_price > Decimal('0'):
                                ...
                                try:
                                    student_group_to_refund = StudentGroup.objects.get(student=student_to_refund, group=attendance.session.group)
                                    student_group_to_refund.balance += price_per_session
                                    student_group_to_refund.save(update_fields=['balance'])
                                    attendance.student_paid_for_session = False
